...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dout1,grads["W2"],grads["b2"] = affine_backward(dout, layer2_cache)
        dx,grads["W1"],grads["b1"] = affine_relu_backward(dout1,layer1_cache)
...
#         outs = []
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization == 'batchnorm':
                ...
                out,bn_cache = affine_bn_relu_forward(out,self.params[W_i], self.params[b_i],self.params['gamma'+str(i+1)],
            ...
            # With layer normalization
                ...
                out,ln_cache = affine_ln_relu_forward(out,self.params[W_i], self.params[b_i],self.params['gamma'+str(i+1)],
        ...
        last_dout,grads["W%d" %(self.num_layers,)],grads["b%d" %(self.num_layers,)] = affine_backward(dout, caches[self.num_layers])
        grads["W%d" %(self.num_layers,)] += self.reg * self.params["W%d" %(self.num_layers,)]
        for j in range(self.num_layers-1,0,-1):
            if self.use_dropout:
                last_dout = dropout_backward(last_dout,caches['dropout'+str(j)])
            
            if self.normalization == 'batchnorm':
                last_dout,grads["W%d" %(j,)],grads["b%d" %(j,)],grads["gamma%d" %(j,)],grads["beta%d" %(j,)] = affine_bn_relu_backward(last_dout,caches[j])
            elif self.normalization == "layernorm":
                last_dout,grads["W%d" %(j,)],grads["b%d" %(j,)],grads["gamma%d" %(j,)],grads["beta%d" %(j,)] = affine_ln_relu_backward(last_dout,caches[j])
            else:  
                last_dout,grads["W%d" %(j,)],grads["b%d" %(j,)] = affine_relu_backward(last_dout,caches[j])
            loss += 0.5 * self.reg * np.sum(self.params["W%d" %(j,)]**2)
            grads["W%d" %(j,)] += self.reg * self.params["W%d" %(j,)]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
